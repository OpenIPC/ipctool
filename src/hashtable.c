...
void ht_iterate_n_erase(
    ...
    for (size_t chain = 0; chain < ht->capacity; chain++) {
        ...
        for (previous = NULL; node;) {
            bool need_erase = callback(node->key, node->value, user);
            if (need_erase) {
                if (previous) {
                    previous->next = node->next;
                } else {
...
size_t _ht_default_hash(const void *raw_key, size_t key_size) {
...
bool _ht_equal(
    const HashTable *table, const void *first_key, void *second_key) {
